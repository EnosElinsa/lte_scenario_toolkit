...
from lte_scenario_toolkit.candidate_cache import CandidateCache, cache_key
...
def test_store_and_load_round_trip_losslessly_without_geometry(tmp_path):
    ...
    assert set(payload) == {"key", "request", "result"}
...
def test_load_quarantines_payload_and_result_invariant_damage(tmp_path, case):
    cache = CandidateCache(tmp_path)
    request = make_request()
    key = key_for(request)
    path = cache.store(key, request, make_result())
