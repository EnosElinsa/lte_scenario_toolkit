...
def main(argv=None):
    ...
    if not output_csv.exists():
        print(f"ERROR: Cannot find CSV: {output_csv}")
        print("Please check if it was generated.")
        return
    ...
    row0 = df.iloc[0]
    rect_info = {
        'rect_id': row0['rect_id'],
        'pt_count': row0['pt_count'],
        'left_x': row0['left_x'],
        'bottom_y': row0['bottom_y'],
        'center_x': row0['center_x'],
        'center_y': row0['center_y']
    }

    import geopandas as gpd
    geometry = gpd.points_from_xy(df['X'], df['Y'])
    selected_points = gpd.GeoDataFrame(df, geometry=geometry, crs="EPSG:3857")

    print(f"   📊 矩形中心: ({rect_info['center_x']:.1f}, {rect_info['center_y']:.1f})")
    print(f"   📊 包含基站: {rect_info['pt_count']} 个")

    if "elevation" in df.columns:
        print(f"   📊 平均高程: {df['elevation'].mean():.1f}m")
...
def render_3d_terrain(rect_info, selected_points, dem, cfg):
    ...
    # 标注基站 (红色柱子加球形顶部，避免地表遮挡，增加可见度)
    z_offset = (np.nanmax(dem_clip) - np.nanmin(dem_clip)) * 0.05
    pole_height = (np.nanmax(dem_clip) - np.nanmin(dem_clip)) * 0.15 # 柱子高度
    valid_mask = ~np.isnan(pt_zs)
    
    px = pt_xs_local[valid_mask]
    py = pt_ys_local[valid_mask]
    pz = pt_zs[valid_mask]
    
    # 绘制每个基站的红色杆子
    for i in range(len(px)):
        ax.plot([px[i], px[i]], [py[i], py[i]], [pz[i], pz[i] + pole_height], color='red', linewidth=2, alpha=0.8, zorder=5)

    # 绘制杆子顶部的圆球
    ax.scatter(px, py, pz + pole_height, c='red', s=60, depthshade=False, edgecolors='white', linewidths=1.2, label=f'Base Station', zorder=6)

    # 增大字体、调整间距
    ax.set_xlabel('X Coordinate (m)', fontsize=18, labelpad=15)
    ax.set_ylabel('Y Coordinate (m)', fontsize=18, labelpad=15)
    ax.set_zlabel('')
    ax.set_zticklabels([])
    ...
    # Determine DCMOP label from rect_size
    dcmop_map = {1000: 'DCMOP1', 2000: 'DCMOP2', 3000: 'DCMOP3'}
    dcmop_label = dcmop_map.get(rect_size, f'{rect_size}m')
    ax.set_title(dcmop_label, fontsize=20, fontfamily='Times New Roman', pad=10)

    ax.legend(loc='upper right', bbox_to_anchor=(0.85, 0.95), prop={'family': 'Times New Roman', 'size': 16})
    
    cbar = fig.colorbar(surf, ax=ax, shrink=0.5, pad=0.04, label='Elevation (m)')
    ...
    plt.savefig(eps_path, format='eps', dpi=300, bbox_inches='tight', pad_inches=0.0)
    plt.close(fig) # 避免阻塞
    print(f"   💾 静态3D图已保存: {png_path}")
    print(f"   💾 EPS图已保存: {eps_path}")
    ...
    fig_plotly.update_layout(
        ...
        scene=dict(
            ...
            xaxis=dict(title_font=dict(family="Times New Roman")),
            yaxis=dict(title_font=dict(family="Times New Roman")),
            zaxis=dict(title_font=dict(family="Times New Roman")),
            aspectmode='manual',
